...
This document defines what BackTrackRTL needs to do and the standards it needs to follow for possible inclusion in PX4. Each requirement should have a clear way to check whether it has been met.
...
BackTrackRTL shall maintain a history of the path previously traveled by the vehicle.

* **FR-1.1:** Each path point shall store the vehicle's three-dimensional local position.
* **FR-1.2:** BackTrackRTL shall record the first valid local position after the vehicle is armed as the first path point.
* **FR-1.3:** After the first path point is recorded, BackTrackRTL shall add a new point only when the vehicle has moved at least the configured minimum distance from the last recorded point.
* **FR-1.4:** BackTrackRTL shall not record new path points while backtracking is active or local position is invalid.
...
* Verify that new path points are not recorded while backtracking is active.
...
BackTrackRTL shall keep the recorded path within its memory limit while preserving the order of the points and meeting the simplification error limit.

* **FR-2.1:** BackTrackRTL shall use a fixed amount of memory for path storage while the vehicle is armed.
* **FR-2.2:** When insufficient path storage remains after permitted simplification, BackTrackRTL shall discard the oldest retained path information as necessary to store new path points.
...
* **FR-2.6:** BackTrackRTL shall mark the recorded path as invalid if a horizontal or vertical local-position reset occurs after path recording has begun.
...
BackTrackRTL shall only activate when the information required to perform backtracking is available.

* **FR-3.1:** BackTrackRTL shall require a valid recorded path before activation.
* **FR-3.2:** BackTrackRTL shall require a valid local position before activation.
...
* **FR-3.4:** BackTrackRTL shall allow activation through PX4's normal mode-selection system.
...
* Request BackTrackRTL with a valid recorded path and valid local position and verify that the vehicle enters BackTrackRTL.
* Attempt activation with an invalid recorded path and verify that BackTrackRTL does not activate.
* Attempt activation with an invalid local position and verify that BackTrackRTL does not activate.
...
* Request BackTrackRTL through PX4's normal mode-selection system and verify that activation occurs only when the activation requirements are satisfied.
* Configure BackTrackRTL as the response for each supported link-loss condition and verify that the corresponding failsafe can request BackTrackRTL.
...
| Supported link-loss conditions that may request BackTrackRTL                                               | FR-3.5                        |
...
| Maximum time allowed to reach a backtracking target                                                        | FR-4.6                        |
...
| Recording behavior after BackTrackRTL exits, including whether recording resumes or a new path is required | FR-1.4, FR-1.6, FR-5.6        |
